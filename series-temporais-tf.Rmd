...
No correlograma da série é possível notar a presença da sazonalidade nos picos múltiplos de 12, como as cavidades tentem a ficar dois quartos atrás dos picos os múltiplos de 6 são os mais negativos. O decaimento muito lento do ACF pode ser sinal de não estacionariedade. Como o decaimento do PACF é menos lento que o do ACF modelos com componentes autorregressivos tem a possibilidade de se adequar melhor. 
...
fit_holt_ad <- hw(train,h=14, seasonal = "additive" ,level=95)
summary(fit_holt_ad)
...
fit_holt_multi <- hw(train,h=14, seasonal = "multiplicative", level=95)
...
AIC(fit_holt_ad$model)
AIC(fit_holt_multi$model)
AIC(ets_model)
...
pander(summary(autoarima_model))
fit_autoarima_model <- forecast(autoarima_model, h=14)

autoplot(train, series ="Série Original") +
...
pander(accuracy(fit_autoarima_model))
...
Na parte não sazonal, o modelo sugere ordem 1 de autorregressividade, 0 de diferenciação e de MA. 
Na parte sazonal, o modelo sugere ordem 2 de autoregressividade, 1 de diferenciação e 0 de MA com 12 observações anuais. 



## Análise de Resíduos ( correlaçaõ) 

Avaliando os resíduos de cada um dos modelos é possível verificar pelos p-values do  Ljung-Box test que o modelo aditivo apresenta um p-value um pouco maior que 0,05, o ETS(A,N,A) possui um p-value maior que 0,05. Ambos possuem resíduos independentes e identicamente distribuídos. No caso do HW multiplicativo o p-value é muito pequeno permitindo rejeitar a hipótese nula, isto é, os resíduos do modelo apresentam problemas de correlação entre os resíduos. O modelo sarima possui um p-value maior que 0,05 e que o modelo ETS. Do ponto de vista  dos resíduos o modelo autoarima parece ser o que melhor se adequa. 
...
checkresiduals(fit_autoarima_model)

```

## Teste de Heterocedasticidade
 
Apenas o modelo Holt Winters multiplicativo se revela heterocedastico.


```{R}

ArchTest(fit_holt_ad$residuals,lags = 12)
ArchTest(fit_holt_multi$residuals,lags = 12)
ArchTest(fit_ets$residuals,lags = 12)
ArchTest(fit_autoarima_model$residuals,lags = 12)
...
O único modelo que passa nos dois testes de normalidade é o SARIMA. 

```{R}

jb.norm.test(fit_holt_ad$residuals, nrepl = 2000)
jb.norm.test(fit_holt_multi$residuals,nrepl = 2000)
jb.norm.test(fit_ets$residuals,nrepl = 2000)
jb.norm.test(fit_autoarima_model$residuals,nrepl = 2000)


shapiro.test(fit_holt_ad$residuals)
shapiro.test(fit_holt_multi$residuals)
shapiro.test(fit_ets$residuals)
shapiro.test(fit_autoarima_model$residuals)
...
Abaixo apresenta-se um resumo dos resultados dos resíduos de cada um dos modelos, é possível verificar que o Holt Winters Multiplicativo não passou em nenhum teste e que os modelos de simplificação exponencial não passaram nos testes de normalidade. O modelo SARIMA passou em todos os testes. 

```{r}

comparativo_residuos <- data.frame(modelo=c("Holt_Winters_Aditivo","Holt_Winters_Multiplicativo","ETS","Auto_Arima_Sazonal"))
comparativo_residuos$LjungBox <- c("S","N","S","S")
comparativo_residuos$Norm_JB <- c("N","N","N","S")
comparativo_residuos$Norm_SH <- c("N","N","N","S")
comparativo_residuos$Homocedasticidade <- c("S","N","S","S")

pander(comparativo_residuos)

```

Abaixo apresenta-se um comparativo dos resultados levando em consideração o AIC, que só pode ser comparado entre modelos da mesma categoria,  e o RMSE. Dos modelos por simplificação exponencial o ETS apresentou o menor AIC e o menor RMSE. Comparando o RMSE do ETS com o SARIMA vemos que o SARIMA se adequa melhor

```{r}

comparativo_indices <- data.frame(modelo=c("Holt_Winters_Aditivo","Holt_Winters_Multiplicativo","ETS","Auto_Arima_Sazonal"))
comparativo_indices$AIC <-  c(AIC(fit_holt_ad$model),AIC(fit_holt_multi$model),AIC(fit_ets$model),AIC(fit_autoarima_model$model))

comparativo_indices$RMSE <- c(data.frame(accuracy(fit_holt_ad))$RMSE,data.frame(accuracy(fit_holt_multi))$RMSE,data.frame(accuracy(fit_ets))$RMSE,data.frame(accuracy(fit_autoarima_model))$RMSE)

pander(comparativo_indices)

```


Face ao exposto, conclui-se que o modelo SARIMA parece se adequar melhor na comparação com os outros. 
